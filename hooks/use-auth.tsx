  ...
  const addCredits = (amount: number, description: string, local: string, category: string) => {
    ...
    if (profile) {
      ...
      if (category === "saude") {
        ...
        stats.securityFeedbackCount = (stats.securityFeedbackCount || 0) + 1
      } else if (category === "administracao") {
        stats.administrationFeedbackCount = (stats.administrationFeedbackCount || 0) + 1
      }
      ...
      if (stats.securityFeedbackCount === 10) {
        gamificationStore.unlockBadge(user.cpf, "security_specialist")
      }

      if (stats.administrationFeedbackCount === 10) {
        gamificationStore.unlockBadge(user.cpf, "administration_specialist")
      }
      ...
      profile.activeChallenges.forEach((challenge) => {
        if (challenge.criteria.type === "feedback_count") {
          ...
          const count =
            category === "saude"
              ? stats.healthFeedbackCount
              : category === "educacao"
                ? stats.educationFeedbackCount
                : category === "seguranca"
                  ? stats.securityFeedbackCount
                  : stats.administrationFeedbackCount

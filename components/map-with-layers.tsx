    ...
    <div className="absolute bottom-5 left-5 z-[1000] bg-white p-3 rounded-md shadow-md">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          {iconsError ? (
            <div dangerouslySetInnerHTML={{ __html: createSvgIcon("#e53935", "H").html }} className="w-6 h-6" />
          ) : (
            <img src="/icons/hospital-marker.png" alt="Saúde" className="w-6 h-6" />
          )}
          <span className="text-xs">Unidades de Saúde</span>
        </div>
        <div className="flex items-center gap-2">
          {iconsError ? (
            <div dangerouslySetInnerHTML={{ __html: createSvgIcon("#1e88e5", "E").html }} className="w-6 h-6" />
          ) : (
            <img src="/icons/school-marker.png" alt="Educação" className="w-6 h-6" />
          )}
...
export default function MapWithLayers({ center, zoom, activeTab, viewMode }) {
  ...
  const getHeatmapPoints = () => {
    const data = activeTab === "saude" ? healthData : educationData
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {viewMode === "markers" && <MarkersLegend />}
    </div>
  )
}
